      ...
      {project.facts && Object.keys(project.facts).length > 0 && (
        <section className="mx-auto max-w-4xl px-6">
          <div className="space-y-6">
            <h2 className="text-3xl font-serif">Project Details</h2>
            <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
              {Object.entries(project.facts).map(([key, value]) => {
                // Format the key for display
                const displayKey = key === 'sqm' ? 'Size' :
                  key === 'yearBuilt' ? 'Year Built' :
                  key.charAt(0).toUpperCase() + key.slice(1).replace(/([A-Z])/g, ' $1');
                
                // Format the value for display
                const displayValue = key === 'sqm' ? `${value} m²` :
                  key === 'bedrooms' ? `${value} ${Number(value) === 1 ? 'Bedroom' : 'Bedrooms'}` :
                  value;
                
                return (
                  <div key={key} className="rounded-xl border border-border/50 bg-card p-4">
                    <dt className="text-xs font-medium uppercase tracking-[0.3em] text-muted-foreground">
                      {displayKey}
                    </dt>
